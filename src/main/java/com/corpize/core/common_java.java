...
public class common_java
{
  ...
  //获取日期值
  public static String getSystemDate(Calendar cal){
    return IntToStr(cal.get(Calendar.YEAR) * 10000 + (cal.get(Calendar.MONTH) + 1) * 100 + cal.get(Calendar.DAY_OF_MONTH));
  }

  public static String getSystemDate(){
    java.util.Calendar cal = Calendar.getInstance();
    return getSystemDate(cal);
  }

  public static String getSystemDateHour(Calendar cal){
    return IntToStr((cal.get(Calendar.MONTH) + 1) * 1000000 + cal.get(Calendar.DAY_OF_MONTH) * 10000 + cal.get(Calendar.HOUR) * 100 + cal.get(Calendar.MINUTE));
  }
